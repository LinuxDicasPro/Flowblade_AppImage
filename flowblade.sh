...
APP=flowblade
BIN="$APP"
DEPENDENCES="ffmpeg frei0r-plugins gmic gtk3 mlt python-cairo python-gobject python-libusb1 python-numpy python-pillow sdl_image swh-plugins python libgirepository gobject-introspection-runtime glib2 glibc pango harfbuzz gdk-pixbuf2 at-spi2-core libimagequant rtaudio libexif abseil-cpp movit sox qt5-base libid3tag qt5-svg md4c double-conversion"
...
function _create_AppRun() {
	...
	prg=io.github.jliljebl.Flowblade
...
BINSAVED="bash bwrap env ffmpeg flowblade gdk-pixbuf-query-loaders python python3 python3.12 python3.12-config python3-config python-config sh tr tty"
...
function _remove_more_bloatwares() {
    ...
    rm -rf ./"$APP".AppDir/.junest/usr/share/bash-completion
    rm -rf ./"$APP".AppDir/.junest/usr/share/pkgconfig
    rm -rf ./"$APP".AppDir/.junest/usr/share/doc
    rm -rf ./"$APP".AppDir/.junest/usr/share/ffmpeg
    rm -rf ./"$APP".AppDir/.junest/usr/share/wayland
    rm -rf ./"$APP".AppDir/.junest/usr/share/X11
    ...
    rm -rf ./"$APP".AppDir/.junest/usr/lib64/qt/mkspecs
    rm -rf ./"$APP".AppDir/.junest/usr/lib64/gnome-settings-daemon-3.0
    rm -rf ./"$APP".AppDir/.junest/usr/lib64/pkgconfig
}
...
mv ./Flowblade_*-x86_64.AppImage ./Flowblade-"$VERSION"-x86_64.AppImage
